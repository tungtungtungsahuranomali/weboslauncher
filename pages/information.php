...
$katList = [];
try {
    $katList = $db->query("SELECT id_kat_info, nm_kat_info FROM kat_info ORDER BY nm_kat_info ASC")->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_info'])) {
        ...
        $katId = !empty($_POST['id_kat_info']) ? (int)$_POST['id_kat_info'] : null;
        $img = $_POST['image_url'] ?? '';
        ...
        $db->prepare("INSERT INTO hotel_info (id_kat_info, title, title_en, description, description_en, icon_path, show_description) VALUES (?, ?, ?, ?, ?, ?, ?)")->execute([$katId, $title, $title_en, $desc, $desc_en, $img, $showDesc]);
    }
    ...
    if (isset($_POST['edit_id'])) {
        ...
        if ($img) {
            $db->prepare("UPDATE hotel_info SET id_kat_info=?, title=?, title_en=?, description=?, description_en=?, icon_path=?, show_description=? WHERE id=?")->execute([$katId, $title, $title_en, $desc, $desc_en, $img, $showDesc, $id]);
        } else {
            $db->prepare("UPDATE hotel_info SET id_kat_info=?, title=?, title_en=?, description=?, description_en=?, show_description=? WHERE id=?")->execute([$katId, $title, $title_en, $desc, $desc_en, $showDesc, $id]);
        }
        echo '<script>window.location.href="admin.php?page=information";</script>';
        exit;
    }
}
...
// Ambil info dengan nama kategori
    ...
    if (isset($_POST['batch_delete'])) {
        ...
        foreach ($ids as $id) {
            $id = (int)$id;
            if ($id > 0) {
                $db->prepare("DELETE FROM hotel_info WHERE id=?")->execute([$id]);
                $deleted++;
            }
        }
        if ($deleted > 0) {
...
$infos = $db->query("SELECT f.*, k.nm_kat_info FROM hotel_info f LEFT JOIN kat_info k ON f.id_kat_info = k.id_kat_info ORDER BY f.id DESC")->fetchAll(PDO::FETCH_ASSOC);
?>
    ...
    <div class="bg-white p-6 rounded shadow">
        <form method="POST" enctype="multipart/form-data" class="space-y-3">
            ...
            <div>
                ...
                <select name="id_kat_info" class="w-full border p-2 rounded">
                    <option value="">-- Tanpa Kategori --</option>
                    <?php foreach ($katList as $k): ?>
                        <option value="<?= $k['id_kat_info'] ?>"><?= htmlspecialchars($k['nm_kat_info']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        ...
        <div class="border-b pb-2 mb-2 flex items-start gap-2">
            ...
            <div class="flex gap-3">
                ...
                <div>
                    ...
                    <?php if (!empty($i['nm_kat_info'])): ?>
                        <span class="text-xs px-2 py-0.5 rounded bg-blue-100 text-blue-800">📂 <?= htmlspecialchars($i['nm_kat_info']) ?></span>
                    <?php else: ?>
...
<div id="editModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50" style="display:none">
    <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        <h3 class="font-bold text-lg mb-4">Edit Info</h3>
        <form method="POST" enctype="multipart/form-data" class="space-y-3">
            <input type="hidden" name="edit_id" id="edit-id">

            <div>
                <label class="block text-sm font-medium mb-1">Kategori</label>
                <select name="id_kat_info" id="edit-kategori" class="w-full border p-2 rounded">
                    <option value="">-- Tanpa Kategori --</option>
                    <?php foreach ($katList as $k): ?>
                        <option value="<?= $k['id_kat_info'] ?>"><?= htmlspecialchars($k['nm_kat_info']) ?></option>
                    <?php endforeach; ?>
...
function edit(data) {
    ...
    document.getElementById('edit-kategori').value = data.id_kat_info || '';
